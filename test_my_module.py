...
class MyModuleTestCase(unittest.TestCase):
    ...
    def test_create_account_2_valid_input(self, mock_input):
        interaction_log, user_input_details = InsertModule.create_account_2(
            self.data_file_path)
    ...
    def test_create_account_2_invalid_phone(self, mock_input):
        ...
        with unittest.mock.patch('sys.stdout', new_callable=StringIO) as fake_stdout:
            InsertModule.create_account_2(self.data_file_path)
    ...
    def test_view_user_details_2_account_number(self, mock_buttonbox, mock_input):
        dummy1, dummy2, found_user_redux = GetModule.view_user_details_2(
            self.data_file_path)
        expected = 'John Doe'
    ...
    def test_update_user_details_2_update_details(self, mock_input, mock_buttonbox):
        interaction_log, user_input_details = UpdateModule.update_user_details_2(
            self.data_file_path)
        self.assertEqual(
    ...
    def test_dete_user(self, mock_input, mock_buttonbox):
        interaction_log, user_input_details = DeleteModule.delete_user_2(
            self.data_file_path)
        self.assertEqual(
